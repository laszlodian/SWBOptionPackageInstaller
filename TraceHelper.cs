...
namespace SWB_OptionPackageInstaller
{
   public class TraceHelper : IDisposable
    {
        const string FILE_LISTENER_NAME = "KUKA_FileListener";
...
#region Singleton 
        ...
        {
            //if (Modell.MeasureCollectionBase.TheMeasures != null)
            //    Trace.TraceInformation(Modell.MeasureCollectionBase.TheMeasures.DetailedTraceInfo);
       ...
       static public void TraceApart(NpgsqlCommand sqlStatement, object classInstance)
       {
        ...
        private void KeepLastTraces(string logFileFullName_in)
        {
            ...
            //Delete last: 
            if (System.IO.File.Exists(GetFullFileNameOf(logFileFullName_in, keepLastTraceNr)))
            {
                System.IO.File.Delete(GetFullFileNameOf(logFileFullName_in, keepLastTraceNr));
            }
        ...
        private void SaveTrace()
        {
            if (_FileListenerActive) //means was active here, because the listener is finalized when the Dispose called by the framework
            {
                ...
                if(System.IO.File.Exists(inputFileName))
                {
                    ...
                    try
                    {
                        System.IO.File.Copy(inputFileName,
                            ...
                            try
                            {
                                System.IO.File.Copy(inputFileName,
                                    string.Format("{0}\\{1}\\{2}_{3}_{4}_{5}.{6}", 
                                        ...
                                        Environment.UserDomainName == null ? string.Empty : "kukauser",
        ...
        void DeleteOldFilesThread()
        {
            try
            {
                var query = Directory.GetFiles(_TraceFileDirectory, string.Format("*.{0}", TRACE_FILE_EXTENSION))
                    .Where(item => DateTime.Now - System.IO.File.GetLastWriteTime(item) > new TimeSpan(3 * 30, 0, 0, 0, 0));
            ...
            {
                Trace.TraceWarning( "Exception at DeteteOldFilesThread:{0}", e.StackTrace);
            }
        } 
        #endregion
