...
namespace SWB_OptionPackageInstaller
{
    ...
    public static class ExtensionMethodsString
    {
                    ...
                    bool emptyLeftAccepted, bool emptyRightAccepted, bool trimInput_in, bool trimOutputs_in, bool moreSeparatorAccepted_in)
        {
            ...
            if (sepPos == -1)
                throw new Exception(string.Format("'{0}' ban  nincs '{1}'",
                    input_in, separator_in));

            if (!moreSeparatorAccepted_in
                && input_in.LastIndexOf(separator_in) != sepPos)
            {
                throw new Exception(string.Format("'{0}' ban  több '{1}' van.",
                    input_in, separator_in));
            }
            ...
            {
                throw new Exception(string.Format("'{0}'-ban a elválasztó ('{1}') elött nincs adat.",
                    input_in, separator_in));
            }

            if (!emptyRightAccepted && right_out == string.Empty)
            {
                throw new Exception(string.Format("'{0}'-ban a elválasztó ('{1}') után nincs adat.",
                    input_in, separator_in));
            }
        }
        ...
        public static string GetItemValue(this string[] items_in, string key_in)
        {
            ...
            if (pair != null)
            {
                if (pair.TryGetParam(key_in, out res))
                    ...
                    throw new Exception(string.Format("pair: {0}; key: {1}", pair.ToString(), key_in));
            }
            else
                throw new Exception(string.Format("'{0}' not found at '{1}'",
